...
public sealed class MeetingValidatorTests
{
    ...
    private readonly CompleteMeetingValidator completeValidator = new();
    ...
    [Fact]
    public void UpdateMeeting_ValidCommand_PassesValidation()
    {
        var command = new UpdateMeetingCommand(1, new UpdateMeetingRequest(
            "Weekly Sync Updated", "Discussion", DateTime.UtcNow, DateTime.UtcNow.AddHours(1), null, null));
    ...
    [Fact]
    public void CancelMeeting_ValidCommand_PassesValidation()
    {
        var command = new CancelMeetingCommand(1);
        var result = cancelValidator.Validate(command);
        Assert.True(result.IsValid);
    }

    [Fact]
    public void CancelMeeting_ZeroId_FailsValidation()
    {
    ...
    [Fact]
    public void CompleteMeeting_ValidCommand_PassesValidation()
    {
        var command = new CompleteMeetingCommand(1);
        var result = completeValidator.Validate(command);
        Assert.True(result.IsValid);
    }

    [Fact]
    public void CompleteMeeting_ZeroId_FailsValidation()
    {
        var command = new CompleteMeetingCommand(0);
        var result = completeValidator.Validate(command);
        Assert.False(result.IsValid);
    }
    ...
    public void UpdateMeetingNotes_InvalidAttendanceStatus_FailsValidation()
    {
        var command = new UpdateMeetingNotesCommand(1, new UpdateMeetingNotesRequest(
            "Notes", new[] { new ParticipantAttendanceUpdate(2, "MAYBE") }));
