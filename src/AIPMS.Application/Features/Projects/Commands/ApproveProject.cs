...
public sealed record ApproveProjectCommand(
    long ProjectId,
    ...
    : IRequestHandler<ApproveProjectCommand, ProjectDto>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!isAdmin && !isStaff)
        {
            throw new ForbiddenException("Only Admin or Department Staff can approve project proposals.");
        }
        ...
        if (!ProjectStateMachine.CanTransition(currentStatus, ProjectStatus.Approved))
        {
            throw new ConflictException($"Cannot transition project from status {project.Status} to APPROVED.");
        }

        // Update project status to APPROVED and write history (all within repository transaction)
