...
BEGIN TRY
    ...
    IF NOT EXISTS (SELECT 1 FROM sys.check_constraints WHERE name = 'ck_projects_proposal_source')
    BEGIN
        ALTER TABLE dbo.projects ADD CONSTRAINT ck_projects_proposal_source CHECK (proposal_source IN ('STUDENT_PROPOSAL', 'PUBLISHED_TOPIC'));
    END;

    IF NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = 'fk_projects_topic')
    BEGIN
        ALTER TABLE dbo.projects ADD CONSTRAINT fk_projects_topic
            FOREIGN KEY (topic_id) REFERENCES dbo.project_topics(id)
